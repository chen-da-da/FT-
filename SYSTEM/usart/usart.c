...
{
    while((USART1->SR&0X40)==0);//循环发送,直到发送完毕 此处修改
    USART1->DR = (u8) ch;
    return ch;
}
#endif
...
void SendData(USART_TypeDef * USARTx, u8 *String,u16 len)
{
    ...
    for(i=0; i<len; i++)
    {
        USART_SendData(USARTx, (u8)String[i]);
...
void uart2_init(u32 bound)
{
    ...
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2,ENABLE);	//使能USART2,PA2->TX,PA3->RX
...
void uart3_init(u32 bound)
{
    ...
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART3,ENABLE);	//使能USART3,PB10->TX,PB11->RX
...
void uart5_init(u32 bound)
{
    ...
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_UART5,ENABLE);	//使能UART5,PC12->TX,PD2->RX
